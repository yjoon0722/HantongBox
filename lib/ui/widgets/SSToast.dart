...
class SSToast {
  ...
  static void show(String msg, BuildContext context,
      {int duration = 1,
        ...
        Border border}) {
    SSToastView.dismiss();
    SSToastView.createView(
        msg, context, duration, gravity, backgroundColor, textColor, backgroundRadius, border);
  }
}

class SSToastView {
  static final SSToastView _singleton = new SSToastView._internal();

  factory SSToastView() {
    return _singleton;
  }

  SSToastView._internal();
  ...
  static void createView(String msg, BuildContext context, int duration, int gravity,
    ...
    _overlayEntry = new OverlayEntry(
      builder: (BuildContext context) => SSToastWidget(
          widget: Container(
    ...
    await new Future.delayed(Duration(seconds: duration == null ? SSToast.LENGTH_SHORT : duration));
    dismiss();
  }
...
class SSToastWidget extends StatelessWidget {
  SSToastWidget({
    Key key,
